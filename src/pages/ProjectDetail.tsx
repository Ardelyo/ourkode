...
export default function ProjectDetail() {
  ...
  return (
    <div ref={containerRef} className="w-full min-h-screen pb-32">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-16 pt-24 article-container relative text-[#1A1A1A] dark:text-[#F5F5F0]">
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
          ...
          <div className="w-full lg:w-2/3 lg:pr-12">
            ...
            <div className="content-block bg-white dark:bg-[#1A1A1A] border border-black/20 dark:border-white/20 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.1)] p-6 md:p-8 mb-20 inline-block w-full">
              <h3 className="font-black text-xl uppercase tracking-tighter mb-6 pb-4 border-b-2 border-black/10 dark:border-white/10 flex items-center gap-3 text-[#1A1A1A] dark:text-[#F5F5F0]">
                <FileText size={24} className="text-emerald-600 dark:text-emerald-400" /> Daftar Isi
              </h3>
              <ul className="space-y-4">
                {project.toc?.map((item, i) => (
                  <li key={i}>
                    <a
                      href={`#${item.id}`}
                      className="font-semibold text-lg text-emerald-700 dark:text-emerald-400 hover:text-black dark:hover:text-white hover:underline underline-offset-4 transition-colors flex items-center gap-3"
                      onClick={(e) => {
                        e.preventDefault();
                        haptics.trigger(50);
                    ...
                    >
                      <span className="text-black/30 dark:text-white/30 font-mono text-sm">0{i + 1}</span>
            ...
            <div className="prose prose-xl prose-stone max-w-none text-[#1A1A1A] dark:text-[#F5F5F0]">
              {project.content?.map((block, index) => {
                switch (block.type) {
                  case 'heading':
                    return (
                      <h2
                        key={index}
                        id={block.id}
                        className="content-block font-black text-4xl md:text-6xl uppercase tracking-tighter mt-24 mb-10 pb-6 border-b-4 border-black dark:border-white/20 flex items-center gap-6 scroll-mt-32 leading-none text-[#1A1A1A] dark:text-[#F5F5F0]"
                      >
                        <span className="text-emerald-600 dark:text-emerald-400 font-mono text-3xl opacity-50 -mt-2">0{(project.toc?.findIndex(t => t.id === block.id) ?? -1) + 1}</span>
                        {block.text}
                      </h2>
                    );
                  case 'paragraph':
                    return (
                      <p key={index} className="content-block text-xl md:text-2xl leading-[1.8] text-black/80 dark:text-white/80 mb-10 font-medium">
                        {block.text}
                      </p>
                    );
                  case 'image':
                    return (
                      <figure key={index} className="content-block my-20 group cursor-pointer relative overflow-hidden">
                        <div className="border-2 border-black dark:border-white/20 p-3 bg-white dark:bg-[#1A1A1A] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.1)] relative z-10 transition-transform duration-500 group-hover:-translate-y-2 group-hover:-translate-x-2">
                        ...
                        <figcaption className="font-mono text-sm uppercase tracking-widest opacity-60 mt-8 text-center flex items-center justify-center gap-3 text-[#1A1A1A] dark:text-[#F5F5F0]">
                  ...
                  case 'quote':
                    return (
                      <blockquote key={index} className="content-block my-20 pl-10 md:pl-16 border-l-[12px] border-emerald-600 dark:border-emerald-500 bg-white dark:bg-[#1A1A1A] p-10 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] dark:shadow-[12px_12px_0px_0px_rgba(255,255,255,0.1)] relative border-2 border-black dark:border-white/20">
                        <div className="absolute -top-8 -left-6 text-8xl md:text-[120px] font-serif text-black/10 dark:text-white/10 leading-none select-none">"</div>
                        <p className="italic text-2xl md:text-4xl font-medium text-black/90 dark:text-white/90 relative z-10 leading-[1.6]">
                          {block.text}
                        </p>
                      </blockquote>
                    );
                    ...
                    return (
                      <ul key={index} className="content-block space-y-6 mb-16 list-none pl-0">
                        {block.items?.map((item, i) => (
                          <li key={i} className="flex items-start gap-6 text-xl md:text-2xl text-black/80 dark:text-white/80 font-medium border-b-2 border-black/10 dark:border-white/10 pb-6 group hover:pl-6 transition-all duration-300">
                            <span className="text-emerald-600 dark:text-emerald-400 mt-1.5 font-mono text-lg font-bold">[{i + 1}]</span>
                  ...
                  case 'code':
                    return (
                      <div key={index} className="content-block my-16 rounded-xl overflow-hidden border-2 border-black dark:border-white/20 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] dark:shadow-[12px_12px_0px_0px_rgba(255,255,255,0.1)] bg-[#1A1A1A]">
          ...
          <div className="w-full lg:w-1/3 relative">
            <div className="sidebar-info flex flex-col gap-10 pb-12 h-[calc(100vh-8rem)] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pr-2 w-full">
              ...
              <div className="bg-white dark:bg-[#1A1A1A] border border-black/20 dark:border-white/20 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.1)] p-8 relative overflow-hidden group text-[#1A1A1A] dark:text-[#F5F5F0]">
                {/* Decorative corner */}
                ...
                <h3 className="font-black text-3xl uppercase tracking-tighter mb-8 pb-4 border-b-2 border-black dark:border-white/20 flex items-center gap-3 text-[#1A1A1A] dark:text-[#F5F5F0]">
                  <FileText className="text-emerald-600 dark:text-emerald-400" /> Spesifikasi
                </h3>

                <table className="w-full text-base">
                  <tbody>
                    <tr className="border-b border-black/10 dark:border-white/10 group/row hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                    ...
                    <tr className="group/row hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                      ...
                      <td className="py-4 px-2 font-bold text-right text-emerald-600 dark:text-emerald-400 hover:underline cursor-pointer truncate max-w-[150px]" title={project.repository}>
              ...
              {/* Categories / Tags */}
              <div className="bg-transparent border-2 border-black/20 dark:border-white/20 border-dashed p-6">
                <h4 className="font-mono text-xs uppercase tracking-widest opacity-50 mb-4 flex items-center gap-2 text-[#1A1A1A] dark:text-[#F5F5F0]">
                  <Tag size={14} /> Kategori Proyek
                </h4>
                <div className="flex flex-wrap gap-2">
                  {project.categories?.map((cat, i) => (
                    <span key={i} className="px-3 py-1 bg-black/5 dark:bg-white/5 font-bold text-sm hover:bg-emerald-600 dark:hover:bg-emerald-500 hover:text-white transition-colors cursor-pointer text-[#1A1A1A] dark:text-[#F5F5F0]">
                      {cat}
                    </span>
                  ))}
                </div>
              ...
              {/* Tech Stack Box */}
              <div className="bg-[#1A1A1A] text-white p-8 shadow-[8px_8px_0px_0px_rgba(5,150,105,1)] relative overflow-hidden border border-black dark:border-white/10">
                {/* Background pattern */}
              ...
              <div className="flex flex-col gap-4 mt-4">
                <div className="flex gap-2">
                  <button onClick={() => haptics.trigger(50)} className="flex-1 border-2 border-black/20 dark:border-white/20 py-3 flex justify-center items-center hover:bg-[#1DA1F2] hover:text-white hover:border-[#1DA1F2] transition-colors group text-[#1A1A1A] dark:text-[#F5F5F0]">
                    <Twitter size={20} className="group-hover:scale-110 transition-transform" />
                  </button>
                  <button onClick={() => haptics.trigger(50)} className="flex-1 border-2 border-black/20 dark:border-white/20 py-3 flex justify-center items-center hover:bg-[#0A66C2] hover:text-white hover:border-[#0A66C2] transition-colors group text-[#1A1A1A] dark:text-[#F5F5F0]">
                    <Linkedin size={20} className="group-hover:scale-110 transition-transform" />
                  </button>
                  <button
                    onClick={handleCopyLink}
                    className={cn(
                      "flex-1 border-2 py-3 flex justify-center items-center transition-colors group text-[#1A1A1A] dark:text-[#F5F5F0]",
                      copied ? "bg-emerald-600 text-white border-emerald-600" : "border-black/20 dark:border-white/20 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black hover:border-black dark:hover:border-white"
                    )}
                  >
                ...
                <a 
                  ...
                  className="w-full bg-emerald-600 text-white py-5 px-8 font-bold text-lg uppercase tracking-widest flex items-center justify-between hover:bg-emerald-700 transition-colors group relative overflow-hidden shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.1)]"
                >
